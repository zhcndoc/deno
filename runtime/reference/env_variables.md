...
title: "环境变量"
description: "在 Deno 中使用环境变量的指南。了解 Deno.env API、.env 文件支持、CLI 配置，以及控制 Deno 行为的特殊环境变量。"
oldUrl:
...
| 名称                   | 描述                                                                                                                                                                                                                                                                                                                                           |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DENO_AUTH_TOKENS       | 用于从私有仓库获取远程模块时使用的 bearer token 和主机名的分号分隔列表<br />(例如 `abcde12345@deno.land;54321edcba@github.com`)                                                                                                                                                                                                                  |
| DENO_TLS_CA_STORE      | 依赖顺序的证书存储的逗号分隔列表。<br />可用值：`system`、`mozilla`。默认值为 `mozilla`。                                                                                                                                                                                                                                                             |
| DENO_CERT              | 从 PEM 编码文件加载证书颁发机构。该文件可以包含多个证书，每个证书位于各自的 PEM 块中。这是 `--cert` 标志的环境变量形式。                                                                                                                                                                                                                         |
...
| DENO_DIR               | 设置缓存目录                                                                                                                                                                                                                                                                                                                               |
| DENO_INSTALL           | 被 [shell 安装脚本](/runtime/getting_started/installation/) 用于选择 `deno` 可执行文件的安装位置。二进制文件位于 `$DENO_INSTALL/bin/deno`。                                                                                                                                                                      |
| DENO_INSTALL_ROOT      | 设置 deno install 的输出目录（默认值为 `$HOME/.deno/bin`）                                                                                                                                                                                                                                                                                   |
| DENO_REPL_HISTORY      | 设置 REPL 历史文件路径；当值为空时，历史文件将被禁用 <br />(默认值为 `$DENO_DIR/deno_history.txt`)                                                                                                                                                                                                                          |
| DENO_NO_PACKAGE_JSON   | 禁用 `package.json` 的自动解析                                                                                                                                                                                                                                                                                                            |
| DENO_CONDITIONS        | 在解析 npm 包导出时使用的自定义条件的逗号分隔列表（等同于 `--conditions` 标志）                                                                                                                                                                                                                           |
| DENO_UNSTABLE_TSGO     | 设置为 `1` 时，使用 TypeScript 的原生（Go）编译器进行类型检查。等同于 `--unstable-tsgo` 标志。参见 [TypeScript](/runtime/fundamentals/typescript/#faster-type-checking-with-the-native-compiler-tsgo)。                                                                                                                                    |
| DENO_NO_PROMPT         | 设置为禁用访问时的权限提示<br />(替代在调用时传递 `--no-prompt`)                                                                                                                                                                                                                                                 |
| DENO_NO_UPDATE_CHECK   | 设置为禁用检查是否有更新版本的 Deno 可用                                                                                                                                                                                                                                                                                          |
| DENO_V8_FLAGS          | 设置 V8 命令行选项                                                                                                                                                                                                                                                                                                                           |
| DENO_JOBS              | 与测试子命令的 `--parallel` 标志一起使用的并行工作线程数量。<br />默认值为可用 CPU 数量。                                                                                                                                                                                                                        |
| DENO_KV_ACCESS_TOKEN   | 连接到 Deno KV 数据库时使用的个人访问令牌（例如通过 [`Deno.openKv`](/api/deno/~/Deno.openKv) 或带有 KV Connect URL 的 `@deno/kv`）。                                                                                                                                                                                       |
| DENO_AUDIT_PERMISSIONS | 审计每一次权限访问。设置为文件路径时将写入 JSONL，或设置为字面量值 `otel` 以通过配置的 OTel 导出器发出 OpenTelemetry 日志记录。有关字段集合请参见 [权限审计](/runtime/reference/permissions/)。                                                                                                      |
...
| DENO_WEBGPU_BACKEND    | 选择 WebGPU 将使用的后端，或按优先级顺序排列的、以逗号分隔的后端列表。可用值为 `vulkan`、`dx12`、`metal` 或 `opengl`                                                                                                                                                                                          |
