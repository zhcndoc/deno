...
Deno 内置的工具让为您的项目设置持续集成（CI）管道变得很容易。[测试](/runtime/test/)、[代码检查和格式化](/runtime/lint_and_format/) 都可以通过相应的命令 `deno test`、`deno lint` 和 `deno fmt` 完成。此外，您还可以在管道中使用 `deno coverage` 根据测试结果生成代码覆盖率报告。
...
在 GitHub 中打开 Actions 标签页，选择“新建工作流程”，搜索“Deno”即可使用该模板。
...
jobs:
  build:
    runs-on: ${{ matrix.os }}
    continue-on-error: ${{ matrix.canary }} # 如果金丝雀版本运行失败，则继续
    strategy:
      matrix:
        os: [ubuntu-latest, macos-latest, windows-latest]
        deno-version: [v2.x]
...
- name: Upload coverage to Coveralls.io
  if: matrix.os == 'ubuntu-latest'
  # 可以使用任何代码覆盖率服务，这里以 Coveralls.io 为示例。
...
为了演示，假设您有一个使用来自
[`@std/log`](/runtime/reference/std/log/) 的日志记录器的项目：
