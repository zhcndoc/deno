---
title: "供应链管理"
description: "让 Deno 依赖保持确定性和安全：锁文件纪律、最低依赖年龄、deno audit、主动更新，以及推荐的 CI 基线。"
---

现代 JavaScript 项目会从许多来源拉取代码（JSR、npm、HTTPS URL、
本地工作区）。良好的供应链管理可以帮助你实现四个目标：
...
1. 有意地固定版本
   ...
   - 对于库，使用 caret 范围（`^1.2.3`）可让使用者获得
     向后兼容的修复。
   - 在生产应用中避免使用无限制（`*`）或过于宽泛的范围。
2. 提交你的 `deno.lock` 文件。
3. 在 CI / 生产环境中启用冻结锁文件（`--frozen` 或
   `"lock": { "frozen": true }`），这样新的、未见过的依赖会导致构建失败，
   而不是悄无声息地出现。
4. 当你需要密封/离线构建时（`"vendor": true`），或者当你
   必须在本地修补第三方代码时，使用 vendoring。vendoring 并不会取消
   锁文件的必要性——它是对锁文件的补充。
5. 在较大的代码库中，优先使用 import map（`imports`）条目，而不是原始 HTTPS 导入，
   以集中管理版本变更。
6. 定期解除冻结并有意识地更新（例如按周或按冲刺节奏），而不是在功能开发期间临时更新。
7. 设置一个[最低依赖年龄](#minimum-dependency-age)，以便新发布的
   版本不会在生态系统有时间发现被破坏的发布之前就进入安装流程。
