...
`node:module` 内置模块包含
[`registerHooks()`](/runtime/reference/loader_hooks/) API，您可以使用它来
自定义程序内部的模块解析和加载。
...
这意味着，包导出中键等于这些字符串中的任意一个时，列表中的第一个条件将被匹配。对于 `require()` 解析，包括 `createRequire()`，条件如下：

```json
["require", "node", "module-sync", "default"]
```

Deno 在分析通过 `require()` 重新导出的 CommonJS 模块时也会应用 `module-sync`。
...
与所有本地 FFI 一样，请传递 `--allow-ffi` 以授予显式权限。请查看[安全性和权限](/runtime/reference/permissions/#ffi-(foreign-function-interface))。

## 从 Node 迁移到 Deno

使用 Deno 运行 Node.js 项目通常几乎不需要更改。有关详细信息、可选工具链改进以及 Node 到 Deno 命令速查表，请参阅 [从 Node.js 迁移到 Deno 指南](/runtime/migrate/)。
