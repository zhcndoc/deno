...
5 字段的 cron 表达式每个时间单位对应一个字段，并用空格分隔：
...
│ │ ┌──────── 日期 (1-31)
│ │ │ ┌────── 月份 (1-12 或 JAN-DEC)
│ │ │ │ ┌──── 星期几 (0-6 或 SUN-SAT，其中 0 表示星期日)
│ │ │ │ │
* * * * *
```

每个字段都可以接受一个精确值、`*`（每个值）、一个范围（`1-5`）、一个列表
（`1,3,5`）或一个步进值（`*/15` 表示每十五个单位一次）。例如，
`0 9 * * MON-FRI` 会在工作日的 UTC 09:00 运行。
