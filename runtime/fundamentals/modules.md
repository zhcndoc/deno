...
支持导入 `text` 和 `bytes` 模块仍处于实验阶段，需要
`--unstable-raw-imports` CLI 标志或
[`deno.json`](/runtime/fundamentals/configuration/) 中的 `unstable.raw-import` 选项。
...
2. 提交你的 `deno.lock` 文件。
3. 在 CI / 生产环境启用冻结锁文件（`--frozen` 或 `"lock": { "frozen": true }`），新出现的未锁定依赖会导致构建失败，而非悄无声息地出现。
