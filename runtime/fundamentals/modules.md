...
title: "模块与依赖项"
description: "Deno 中模块和依赖项管理指南。了解 ECMAScript 模块、第三方包、导入映射、依赖管理、版本控制，以及如何发布你自己的模块。"
oldUrl:
...
重新映射的名称可以是任何有效的说明符。这是 Deno 中一个非常强大的功能，可以重映射任何内容。了解更多信息，请参阅
...
使用 `deno add` 子命令可以轻松完成安装过程。它将自动把你请求的包的最新版本添加到 `deno.json` 的 `imports` 部分。
...
传递给 `deno add` / `deno install` 的未加前缀包名默认会被视为 npm 包。`deno add express` 现在等同于
`deno add npm:express`。JSR 包仍然需要 `jsr:` 前缀以保持明确无歧义。

:::

更多内容请阅读 [`deno add` 参考文档](/runtime/reference/cli/add/)。
...
可以为你正在导入的包指定版本范围。这是通过使用 `@` 符号后跟版本范围说明符来完成的，并遵循 [semver](https://semver.org/) 版本控制方案。如果你需要在多个工作区成员之间共享单个版本范围，请参阅
...
注意：如果你需要缓存或本地修改依赖以用于多次构建，可以考虑[供应远程模块](#供应-远程模块)。
...
- 当 `"nodeModulesDir": "auto"`：目录会在每次运行时重建，启动时间稍有增加，但确保始终使用最新版本。
- 当 `"nodeModulesDir": "manual"`（使用 `package.json` 时的默认值）：更新包后，你必须运行 `deno install`，将更改同步到工作区的 `node_modules` 目录。
...
将上述代码段添加到你的 `deno.json` 文件中，Deno 将在项目运行时将所有依赖项缓存到 `vendor` 目录中，或者你可以选择运行 `deno install --entrypoint main.ts` 命令以立即缓存依赖项：
...
- [JSR](https://jsr.io) - 推荐，原生支持 TypeScript，并会
  自动为你生成文档
- [npm](https://www.npmjs.com/) - 使用
  [`deno pack`](/runtime/reference/cli/pack/)（Deno 2.8+）从 Deno 项目构建一个
  兼容 npm 的 tarball，或者使用
  [dnt](https://github.com/denoland/dnt) 以获得更可配置的构建流程
- [deno.land/x](https://deno.com/add_module) - 对于 HTTPS 导入，如果可能请改用 JSR
...
- **`deno install --prod`** — 跳过 `package.json` 中的 `devDependencies`。你
  还可以传入 `--skip-types` 以进一步排除 `@types/*` 包。
- **`deno install --entrypoint`** — 仅安装指定入口文件实际（传递地）导入的
  依赖。当与 `--prod` 结合使用时，纯类型依赖也会从
  模块图中排除。
...
1. 有意固定版本
   ...
   - 对库，使用脱字符范围（`^1.2.3`）可让使用者获得
     向后兼容的修复。
   - 避免在生产应用中使用无限制（`*`）或过于宽泛的范围。
2. 提交你的 `deno.lock` 文件。
3. 在 CI / 生产环境中启用冻结锁文件（`--frozen` 或
   `"lock": { "frozen": true }`），这样新的、未见过的依赖会使构建失败，
   而不是悄然出现。
4. 当你需要封闭式/离线构建（`"vendor": true`）或需要在本地修补第三方代码时，使用供应。供应不会取代锁文件，而是与之互补。
5. 在较大的代码库中，优先使用导入映射（`imports`）条目而不是原始 HTTPS 导入，以集中管理版本变更。
6. 定期有意识地解除冻结并更新（例如按周或按冲刺节奏），而不是在功能开发期间临时更新。
7. 设置[最小依赖年龄](#minimum-dependency-age)，以便新发布的版本在生态系统有时间发现被破坏的版本之前，无法进入安装流程。

### 最小依赖年龄

Deno 可以拒绝安装任何比配置年龄更新的包版本。这是对 npm 供应链攻击一种低成本、广泛的防护：恶意版本通常会在几天内被发现并撤回，因此通过类似窗口延迟安装可以拦截其中大部分。
...
- **CLI 标志**，按需应用，例如一次性安装或在 CI 步骤中：

  ```sh
  deno install --minimum-dependency-age=P3D
  ```

- **`.npmrc`**（Deno 2.8+），符合 npm 约定，在 npm 和 Deno 工具链之间共享同一个 `.npmrc` 时很有用。npm 设置只接受整天数：
...
`P3D`（3 天）或 `PT72H`（72 小时），也可以接受整数（按分钟解释）、
绝对截止日期（`2025-09-16`）或 RFC3339 时间戳，或 `0` 表示禁用。有关 Deno 读取的其他 npm 注册中心选项，请参见
[`.npmrc` 配置](/runtime/fundamentals/node/#npmrc-configuration)。

### 典型 CI 模式

在 Deno 2.8+ 中，单命令 [`deno ci`](/runtime/reference/cli/ci/)
...
如果你依赖 `npm` 包（存在 `package.json`），请在运行测试前将 `deno install`
（或 `deno ci`）加入 CI，以便确定性地生成 `node_modules` 目录。
...
提交 `deno.json`、`deno.lock` 和（如使用供应）完整 `vendor/` 目录。

:::tip 自动化每周依赖刷新流程：安排 CI 定时任务，解冻，执行 `deno add --latest`（或手工更新关键包），运行测试，提交包含更新锁文件（和供应目录）的 PR。此举在维护安全补丁流动同时，保持日常构建确定性。 :::
