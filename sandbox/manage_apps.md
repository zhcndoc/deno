...
`sandbox.deno.deploy()` 方法可用于将资源从 sandbox 发布到现有的 Deno Deploy 应用。这允许你将 sandbox 用作托管在 Deno Deploy 上应用的部署管道。
...
- 将这些 API 与 `sandbox.deno.deploy()` 配合使用，可从 sandbox 试验中初始化应用，然后持续管理它们。
