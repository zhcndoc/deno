...
title: 可观测性
description: "Deno Deploy 中监控功能的全面概览，包括日志、跟踪、指标和过滤选项。"
---
...
跟踪列表显示顶层请求跟踪。您通过手动监控创建的自定义跨度会作为其所在请求跟踪下嵌套的子跨度出现，而不是作为列表中的单独条目。要查找它们，请打开相关的请求跟踪，而不是查找独立跟踪。

点击任一跟踪会打开跟踪覆盖抽屉，以瀑布图视图显示该跟踪中的所有跨度。此可视化会按父跨度分组显示每个跨度的开始时间、结束时间和持续时间，根跨度位于顶部。
