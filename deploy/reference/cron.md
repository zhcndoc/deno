...
[`Deno.cron()`](/runtime/fundamentals/cron/) 是 Deno 运行时的一个 API——它随 Deno 自带。Deno Deploy 在该运行时 API 之上构建：它会在部署时发现您的 `Deno.cron()` 定义，调度并调用它们，处理重试，并在仪表盘和日志中展示运行情况，因此您无需自己维持一个长期运行的进程。
